...
export class AuthEffects {
  ...
  getAuthStatus$ = createEffect(() => this.auth.getStatus().pipe(map(userOrNull => AuthApiActions.getAuthStatusSuccess(userOrNull)))
  );

  login$ = createEffect(() => this.actions$.pipe(
    ofType(AuthUserActions.loginUser),
    concatMap(action => this.auth.login(action.username, action.password).pipe(
      mergeMap((user) => {
        const actions = [];
